    ...
    D: Backend<B> + 'static,
{
    ...
    pub fn get_by_id(&self, hash: BlockId<B>) -> Result<Option<RuntimeVersion>> {
        ...
        if self.versions.load().contains_key(&code_hash) {
            ...
            ext.register_extension(ReadRuntimeVersionExt::new(self.exec.clone()));
    ...
    pub fn get(&self, hash: B::Hash) -> Result<Option<RuntimeVersion>> {
        ...
        self.get_by_id(BlockId::hash(hash))
    }
}
