...
/// Full client type.
pub type TFullClient<TBl, TExecDisp, TRA> =
    Client<TFullBackend<TBl>, TFullCallExecutor<TBl, TExecDisp>, TBl, TRA>;
...
type TFullParts<TBl, TExecDisp, TRA> = (
    TFullClient<TBl, TExecDisp, TRA>,
    // Arc<TFullBackend<TBl>>,
    TaskManager,
);

/// Creates a new full client for the given config.
pub fn new_full_client<TBl, TExecDisp, TRA>(
    config: &Configuration,
    telemetry: Option<TelemetryHandle>,
    executor_send_client: TracingUnboundedSender<u32>,
    backend: Arc<TFullBackend<TBl>>,
) -> Result<TFullClient<TBl, TExecDisp, TRA>, Error>
where
    TBl: BlockT,
    TExecDisp: NativeExecutionDispatch + 'static,
    NumberFor<TBl>: Into<u32>,
{
    new_full_parts(config, telemetry, executor_send_client, backend).map(|parts| parts.0)
}

/// Create the initial parts of a full node.
pub fn new_full_parts<TBl, TExecDisp, TRA>(
    config: &Configuration,
    telemetry: Option<TelemetryHandle>,
    executor_send_client: TracingUnboundedSender<u32>,
    backend: Arc<TFullBackend<TBl>>,
) -> Result<TFullParts<TBl, TExecDisp, TRA>, Error>
where
    TBl: BlockT,
    TExecDisp: NativeExecutionDispatch + 'static,
    NumberFor<TBl>: Into<u32>,
