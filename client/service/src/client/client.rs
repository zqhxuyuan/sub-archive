...
use sp_api::{
    ...
    Metadata as MetadataApi, ProvideRuntimeApi,
};
...
where
    ...
    <Client<B, E, Block, RA> as ProvideRuntimeApi<Block>>::Api: MetadataApi<Block>,
{
    fn lock_import_and_run<R, Err, F>(&self, f: F) -> Result<R, Err>
    where
        F: FnOnce(&mut ClientImportOperation<Block, B>) -> Result<R, Err>,
    ...
    // C::StateBackend: sp_api::StateBackend<sp_api::HashFor<Block>,>
{
    ...
    pub fn key_changes_proof_with_cht_size(
            ...
            fn root(
                ...
                if block < self.min {
                    if let Some(ref root) = root {
                        self.required_roots_proofs.lock().insert(block, *root);
                    }
                }
                Ok(root)
            }
        ...
        for (config_zero, config_end, config) in configs {
            ...
            .map_err(sp_blockchain::Error::ChangesTrieAccessFailed)?;
        ...
        match (
            ...
            (true, ref mut storage_changes @ None, Some(ref body)) => {
                ...
                if elapsed.as_millis() > 1000 {
                    info!(
                        "client 🐢  importing {} execute block cost:{:?}",
                ...
                if elapsed.as_millis() > 1000 {
                    info!(
                        "client 🐢  importing {} state_at and get storage, cost:{:?}",
                        import_block.header.number(),
                        now.elapsed()
                    );
                }
...
where
    ...
    NumberFor<Block>: Into<u32>,
    Client<B, E, Block, RA>: ProvideRuntimeApi<Block>,
    <Client<B, E, Block, RA> as ProvideRuntimeApi<Block>>::Api: CoreApi<Block> + MetadataApi<Block>,
{
    fn insert_aux<
        'a,
        'b: 'a,
...
impl<B, E, Block, RA> sp_consensus::block_validation::Chain<Block> for Client<B, E, Block, RA>
where
    B: backend::Backend<Block> + 'static,
    E: CallExecutor<Block>,
    Block: BlockT,
